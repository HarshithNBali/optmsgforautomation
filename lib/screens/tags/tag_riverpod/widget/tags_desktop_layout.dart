...
class TagsDesktopLayout extends ConsumerWidget {
  final TagsState state;
  final TagsNotifier notifier;
  final TextEditingController tagController;
  final WidgetRef parentRef;

  const TagsDesktopLayout({
    super.key,
    required this.state,
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final bool canShowToggle = kIsWeb;
    ...
    return Column(
      ...
      children: [
        ...
        Container(
          ...
          child: Row(
            children: [
              ...
              const Spacer(),
              // Batch delete when in multi-select
        ...
        // Header with checkbox toggle + title + counter
        Padding(
...
/// Stateful list view that supports drag-to-select and keyboard modifiers.
class _TagsListView extends StatefulWidget {
  final TagsState state;
  final TagsNotifier notifier;
  ...
  @override
  State<_TagsListView> createState() => _TagsListViewState();
}

class _TagsListViewState extends State<_TagsListView> {
  ...
  void _handleTap(int index) {
    ...
    if (isShift) {
      int from;
      if (widget.state.lastClickedIndex != -1) {
        from = widget.state.lastClickedIndex;
      } else {
        from = index;
      }
                                ...
                                : Row(
                                    ...
                                    children: [
                                      ...
                                      IconButton(
                                        ...
                                        onPressed: () =>
                                            widget.notifier.deleteTag(tag.id),
                                        padding: EdgeInsets.zero,
                                        constraints: const BoxConstraints(),
                                      ),
                                    ],
